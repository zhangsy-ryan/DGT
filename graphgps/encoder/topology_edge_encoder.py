...
class RingEdgeEncoder(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        e2e_ring_dense = 2 * to_dense_adj(batch.e2e_ring_index, batch=batch.e_batch).long()
        e2e_ring_dense -= to_dense_adj(batch.e2e_edge_index, batch=batch.e_batch).long()
        e2e_ring_dense[e2e_ring_dense == -1] = 0
        batch.e2e_edge_dense += self.e2e_encoder(e2e_ring_dense)
